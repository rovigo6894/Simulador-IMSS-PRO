...
with tab2:
    ...
    if st.button("Calcular Modalidad 40", type="primary", use_container_width=True, key="btn2"):
        with st.spinner("Analizando..."):
            ...
            # Comparativa: Cuadro blanco (BASE) + Cuadro azul (CON M40)
...
with tab3:
    ...
    if st.button("Comparar todos los escenarios", type="primary", use_container_width=True, key="btn3"):
        with st.spinner("Generando comparativa..."):
            ...
            st.subheader("📊 Comparativa: Pensión Base vs Con M40")
            
            fig = go.Figure(data=[
                go.Bar(name='Sin M40', x=[f"{m} meses" for m in meses_lista], 
                       y=[pension_base]*len(meses_lista), 
                       marker_color='#999999',
                       text=[f"${pension_base:,.0f}" for _ in meses_lista],
                       textposition='outside'),
                go.Bar(name='Con M40', x=[f"{m} meses" for m in meses_lista], 
                       y=pensiones_con_m40, 
                       marker_color='#0066b3',
